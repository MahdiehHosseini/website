    ...
    return(
        <nav className="flex justify-between md:justify-end z-10">
            <span onClick={()=>setShowNav(!showNav)} className={`${showNav ? 'mt-5' : 'mt-7'} md:hidden flex flex-col justify-between h-4 text-xl mt-7 ml-5 z-10 cursor-pointer`}>
                <span className={`${showNav ? 'rotate-45 mt-3' : ''} border border-${them.mainTxtColor} w-10 h-0`} />
                <span className={`${showNav ? '-rotate-45' : ''} border border-${them.mainTxtColor} w-10 h-0`} />
            </span>
            ...
            <div className={`${!showNav && 'hidden'} flex h-screen w-full md:border-l border-${them.subBorderColor} md:flex md:mt-28 md:right-0 absolute md:w-1/6 flex-col justify-around md:bg-inherit py-52 md:p-5 md:h-4/6 items-center bg-${them.mainBgColor}/70`}>
                ...
                <Link to="/skills">
                    <FaClipboardList onClick={() => setShowNav(false)} className="text-4xl" />
                </Link>
                <Link to="/portfolio">
                    <FaClone onClick={() => setShowNav(false)} className="text-4xl" />
                </Link>               
                <Link to="/contact">
                    <FaEnvelope onClick={() => setShowNav(false)} className="text-4xl" />
                </Link>
            </div>
            ...
            <span className="mt-4 flex flex-col items-center mr-5 md:mr-10 lg:mr-16 2xl:mr-20">
                <span onClick={()=> lang === 'EN' ? dispatch(toggleLang('FA')) : dispatch(toggleLang('EN'))} className=" cursor-pointer text-2xl flex">
                    <p className={`mr-1 ${lang !== 'EN' && `text-${them.subTxtColor}` }`}>EN</p>
                     / 
                    <p className={`ml-1 ${lang !== 'FA' && `text-${them.subTxtColor}` }`}>FA</p>
                </span>
                <span onClick={()=> them.mainTxtColor === 'white' ? dispatch(toggleMode('light')) : dispatch(toggleMode('dark'))} className=" cursor-pointer items-center text-2xl flex mt-2">
                    <FaSun className={`mr-1 ${them.mainTxtColor === 'white' && `text-${them.subTxtColor}`}`} />
                     /  
                    <FaMoon className={`ml-1 ${them.mainTxtColor === 'black' && `text-${them.subTxtColor}`}`} />
                </span>
            </span>
        </nav>
    )
